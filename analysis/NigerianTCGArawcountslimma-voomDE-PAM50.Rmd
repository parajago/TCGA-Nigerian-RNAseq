...
setwd("~/Research-Local/2020-rnaseq/Inputs/NigerianTCGA_quants-proteincoding")
```
...
FOLDER <- "/Users/parajago/Research-Local/2020-rnaseq/Inputs/NigerianTCGA_quants-proteincoding"
...
t_norm_countmatrix_temp <- subset(t_norm_countmatrix, t_norm_countmatrix$condition1!="TCGA_other")

quant.pca <- prcomp(t_norm_countmatrix_temp[,1:19724])
autoplot(quant.pca, data=t_norm_countmatrix_temp, colour='sampleCondition', shape='condition1', main="PCA of quantile normalization results prior to DE analysis")
```
...
```{r Specific gene comparison, message=FALSE, warning=FALSE, echo=FALSE, results=FALSE}
...
#TNFSF10 expression within Nigerians by subtype
sampleTableTNF <- data.frame(sampleName=gsub(".htseq.counts","",sampleFiles),
                          fileName=sampleFiles,
                          condition1=sampleConditionrace,
                          condition2=sampleConditionPAM50)
sampleTableTNF <- sampleTableTNF %>% dplyr::filter(condition1=="Nigerian")
sampleTableTNF$condition1 <- NULL

colnames(sampleTableTNF) <- c("sampleName", "fileName", "sampleCondition")

ddsHTSeqMFTNF <- DESeqDataSetFromHTSeqCount(sampleTable=sampleTableTNF,
                                       directory=FOLDER,
                                       design=~sampleCondition)

ddsHTSeqMFTNF <- ddsHTSeqMFTNF[rowSums(counts(ddsHTSeqMFTNF)) > 0, ]
ddsTNF <- estimateSizeFactors(ddsHTSeqMFTNF)
TNF <- plotCounts(ddsTNF, gene = "ENSG00000121858.6", intgroup=c("sampleCondition"), returnData = TRUE)

ggplot(TNF, aes(x = sampleCondition, y = count, fill=sampleCondition)) + 
  geom_boxplot() +
  theme_bw() +
  scale_fill_manual(values=c("#FF0000", "#FFFF00", "#00FF00", "#00FFFF", "#0033FF")) + 
  coord_trans(y="log2") + 
  ggtitle("Distribution of TNFSF10 expression across Nigerian subtypes") +
...
ggplot(temp_vegf_countmatrix, aes(x=condition2, y=signature)) + geom_boxplot() + xlab("Breast cancer subtype") + ylab ("VEGF signature") + theme_bw()
...
ggplot(temp_claudin_countmatrix, aes(x=condition2, y=signature)) + geom_boxplot() + xlab("Breast cancer subtype") + ylab ("Claudin signature")+theme_bw()
