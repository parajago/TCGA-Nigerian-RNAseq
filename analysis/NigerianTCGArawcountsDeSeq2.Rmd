...
This is a check between the TCGA black and white patients to verify this differential expression analysis based on prior work by Stewart et al (PLOS One 2013) and Huo et al (JAMA Oncology 2017). 
...
diffTCGA2<- results(ddsMFbe, contrast=c("sampleCondition", "TCGA_black.LumA", "TCGA_white.LumA"), alpha=0.05)
diffTCGA3<- results(ddsMFbe, contrast=c("sampleCondition", "TCGA_black.LumB", "TCGA_white.LumB"), alpha=0.05)
...
diffTCGAhead <- head(diffTCGA[ order(diffTCGA$log2FoldChange, diffTCGA$padj), ],100)
diffTCGAtail <- tail(diffTCGA[ order(diffTCGA$log2FoldChange, diffTCGA$padj), ],100)
write.table(diffTCGAhead,"TCGAblack-TCGAwhite-DiffExpGenes100-Head",sep="\t",row.names=TRUE)
write.table(diffTCGAtail,"TCGAblack-TCGAwhite-DiffExpGenes100-Tail",sep="\t",row.names=TRUE)
...
#This synthesizes all of the significant findings that we have collected
basalbreast <- rbind(resBasalhead, resBasaltail, resBasalblackhead, resBasalblacktail)
her2breast <- rbind(resHer2head, resHer2tail)
lumAbreast <- rbind(resLumAhead, resLumAtail, resLumAblackhead, resLumAblacktail)
lumBbreast <- rbind(resLumBhead, resLumBtail, resLumBblackhead, resLumBblacktail)
```
