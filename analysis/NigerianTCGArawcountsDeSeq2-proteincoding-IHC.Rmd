...
sampledriver <-sampleFiles
countVar2=1
for (sample in sampledriver){
  if (stri_detect_fixed(sample,"driversilent")==TRUE){
    sampledriver[countVar2] <- "silent"
    countVar2=countVar2+1
  }
  else if (stri_detect_fixed(sample,"driverTP53")==TRUE){
    sampledriver[countVar2] <- "TP53"
    countVar2=countVar2+1
  }
  else if (stri_detect_fixed(sample,"driverGATA3")==TRUE){
    sampledriver[countVar2] <- "GATA3"
    countVar2=countVar2+1
  }
  else if (stri_detect_fixed(sample,"driverdual")==TRUE){
    sampledriver[countVar2] <- "dual"
    countVar2=countVar2+1
  }
  else{
    sampledriver[countVar2] <- NA
    countVar2=countVar2+1
  }
}
...
for (sample in sampleConditionPAM50){
  if (stri_detect_fixed(sample,"PAMH")==TRUE){
    sampleConditionPAM50[countVar2] <- "Her2"
    countVar2=countVar2+1
  } else if (stri_detect_fixed(sample,"PAMB")==TRUE){
    sampleConditionPAM50[countVar2] <- "Basal"
    countVar2=countVar2+1
  } else if (stri_detect_fixed(sample,"PAMA")==TRUE){
    sampleConditionPAM50[countVar2] <- "LumA"
    countVar2=countVar2+1
  } else if (stri_detect_fixed(sample,"PAMLB")==TRUE){
    ...
    sampleConditionPAM50[countVar2] <- "PAM0"
    countVar2=countVar2+1
  }
}
...
sampleTable <- data.frame(sampleName=gsub("htseq.counts","",sampleFiles),
                          ...
                          condition7=sampledriver,
                          condition8=sampleConditionPAM50,
                          batch=batchval)
...
with(restemp, plot(log2FoldChange, -log10(padj), pch=20, main="Volcano plot of differential gene expression between \nbreast cancers in Nigerian patients with and without 'silent genomes'", xlim=c(-7.5,7.5), ylim=c(1,8)))
with(subset(restemp, padj<0.05 & (2^(abs(log2FoldChange))>15)), points(log2FoldChange, -log10(padj), pch=20, col="blue"))
with(subset(restemp, padj<0.05 & (2^(abs(log2FoldChange))>15)), textxy(log2FoldChange, -log10(padj), labs=symbol, cex=.5))
...
plotCounts(ddssil, gene="ENSG00000091831.17", intgroup=c("sampleCondition"), main="Distribution of ESR1 expression by silent genome status")

plotCounts(ddssil, gene="ENSG00000121879.3", intgroup=c("sampleCondition"), main="Distribution of PIK3CA expression by silent genome status")

#write.csv(diffsiltable, file = "RNA-DE-Nigerian-silent.csv", row.names =FALSE)
```
We found 189 significantly differentially expressed genes between those patients with silent genomes and non-silent genomes among the Nigerian patients. We were able to use DESeq2 for this process as this comparison was made only within the Nigerian patients.
...
#Driver-based PCA and comparison
sampleTabledriver <- data.frame(sampleName=gsub(".htseq.counts","",sampleFiles),
                          fileName=sampleFiles,
                          sampleCondition=sampledriver)

sampleTabledriver <- sampleTabledriver %>% subset(is.na(sampleCondition)==FALSE)

ddsHTSeqdriver <- DESeqDataSetFromHTSeqCount(
                                      sampleTable=sampleTabledriver,
                                      directory=FOLDER,
                                      design=~sampleCondition)

ddsHTSeqdriver <- ddsHTSeqdriver[rowSums(counts(ddsHTSeqdriver)) > 0, ] #Pre-filtering the dataset by removing the rows without any  information about gene expression

ddsHTSeqdriver <- estimateSizeFactors(ddsHTSeqdriver) #The size factor is the median ratio of the sample over a "pseudosample": for each gene, the geometric mean of all samples. This accounts for sequencing depth. 

vsddriver <- vst(ddsHTSeqdriver, blind=TRUE) #Variance-stabilizing transformation, only using this since >50 samples

plotPCA(vsddriver, intgroup=c("sampleCondition")) + labs(title="PCA of Nigerian patient RNAseq data based on primary driver")

sampleTabledriverIHC <- data.frame(sampleName=gsub(".htseq.counts","",sampleFiles),
                          fileName=sampleFiles,
                          condition1=sampledriver,
                          condition2=sampleConditionIHC)
